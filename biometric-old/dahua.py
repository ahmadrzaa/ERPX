...
class DahuaBiometric:
    ...
    def add_user_via_cgi(self, user_id, name, card_number):
        ...
        payload = {
            "userID": user_id,
            "userName": name,  # Updated parameter to match common Dahua convention
            "cardNo": card_number,
            "roleID": 1  # Adding roleID as it might be required
        }
        response = self.session.post(endpoint, data=urlencode(payload), auth=self.auth)
        if response.status_code == 200:
    ...
    def update_user_via_cgi(self, user_id, name=None, card_number=None):
        ...
        payload = {}
        if name:
            payload["userName"] = name  # Updated parameter to match common Dahua convention
        if card_number:

...
from erpx_automations.methods.methods import get_model_class
from erpx_views.cbv_methods import render_template

"""Objectives and key result section"""


class Period(erpxModel):
...
class KeyResult(erpxModel):
    ...
    history = erpxAuditLog(bases=[erpxAuditInfo])
...
class EmployeeObjective(erpxModel):
    ...
    history = erpxAuditLog(bases=[erpxAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
...
class Comment(models.Model):
    ...
    history = erpxAuditLog(excluded_fields=["comment"], bases=[erpxAuditInfo])
    objects = erpxCompanyManager(
...
class QuestionTemplate(erpxModel):
...
class Question(erpxModel):
    ...
    objects = erpxCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(erpxModel):
    ...
    objects = erpxCompanyManager("question_id__template_id__company_id")


class Feedback(erpxModel):
...
class EmployeeBonusPoint(erpxModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,

...
from base.erpx_company_manager import erpxCompanyManager
...
from erpx_audit.methods import get_diff
from erpx_audit.models import erpxAuditInfo, erpxAuditLog
...
class Grade(models.Model):
    """
    Model to define grade labels and corresponding values.
    """
    label = models.CharField(max_length=100, verbose_name=_("Grade Label"))
    value = models.PositiveIntegerField(verbose_name=_("Grade Value"))

    def __str__(self):
        return f"{self.label} ({self.value})"
    def get_grade_choices():
        return [(grade.value, grade.label) for grade in Grade.objects.all()]
    def save(self, *args, **kwargs):
        """
        Override the save method to automatically generate 10 steps for each grade.
        """
        is_new = self.pk is None  # Check if the grade is new
        super().save(*args, **kwargs)
        if is_new:
            # Automatically create 10 steps for this grade
            for step_number in range(1, 11):
                GradeStep.objects.create(
                    grade=self,
                    step=step_number,
                    amount=self.value + (step_number - 1) * 20,  # Example increment logic
                )

    class Meta:
        verbose_name = _("Grade")
        verbose_name_plural = _("Grades")
        ordering = ["value"]  # Order by grade value


class GradeStep(models.Model):
    """
    Model to define steps within a grade and their corresponding amounts.
    """
    ...
    def __str__(self):
        return f"{self.grade.label} - Step {self.step} ({self.amount})"
    @staticmethod
    def get_step_choices(grade_id):
        """
        Retrieve all step choices for a given grade.
        Args:
            grade_id: The ID of the grade.
        Returns:
            List of tuples (step, label) for all steps under the specified grade.
        """
        steps = GradeStep.objects.filter(grade_id=grade_id).order_by("step")
        return [(step.id, f"{step.grade.label} & Step-{step.step}") for step in steps]

    @staticmethod
    def get_grade_step_value(grade_id, step_id):
        """
        Retrieve the amount for the given grade and step combination.
        """
        try:
            grade_step = GradeStep.objects.get(grade_id=grade_id, step=step_id)
            print(f"Found GradeStep: Grade={grade_step.grade.label}, Step={grade_step.step}, Amount={grade_step.amount}")
            return grade_step.amount
        except GradeStep.DoesNotExist:
            print(f"GradeStep not found for Grade ID={grade_id} and Step ID={step_id}")
            return None
        class Meta:
            unique_together = ("grade", "step")
            ordering = ["grade", "step"]
            verbose_name = _("Grade Step")
            verbose_name_plural = _("Grade Steps")
class Employee(models.Model):
    ...
    # Use models.CharField for the nationality field
    ...
    def get_archive_condition(self):
        ...
        if apps.is_installed("onboarding"):
            OnboardingStage = get_erpx_model_class("onboarding", "onboardingstage")
            OnboardingTask = get_erpx_model_class("onboarding", "onboardingtask")
        ...
        if apps.is_installed("recruitment"):
            Recruitment = get_erpx_model_class("recruitment", "recruitment")
            Stage = get_erpx_model_class("recruitment", "stage")
    ...
    def check_online(self):
        ...
        """
        if apps.is_installed("attendance"):
            Attendance = get_erpx_model_class("attendance", "attendance")
...
class EmployeeBankDetails(erpxModel):
    """
...
class PolicyMultipleFile(erpxModel):
    """
...
class DisciplinaryAction(erpxModel):
    """
    Disciplinary model
    """
...
class EmployeeGeneralSetting(erpxModel):
    """

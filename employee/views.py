...
from erpx.filters import erpxPaginator
from erpx.group_by import group_by_queryset
from erpx.erpx_settings import erpx_DATE_FORMATS
from erpx.methods import get_erpx_model_class
from erpx_audit.models import AccountBlockUnblock, HistoryTrackingFields
from erpx_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
...
@login_required
@manager_can_enter("erpx_documents.view_documentrequests")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="erpx_documents.view_documentrequests",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("erpx_documents.add_documentrequests")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "erpx_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "erpx_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("erpx_documents.change_documentrequests")
...
@login_required
@hx_request_required
@owner_can_enter("erpx_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("erpx_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("erpx_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("erpx_documents.add_document")
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = erpxPaginator(qryset, get_pagination())
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_erpx_model_class(
                    ...
                    OnboardingStage = get_erpx_model_class(
                    ...
                    OnboardingTask = get_erpx_model_class(
    ...
    for column_value, column_name in selected_columns:
        ...
        for employee in employees:
            ...
            if type(value) == date:
                ...
                for format_name, format_string in erpx_DATE_FORMATS.items():
...
def dashboard_employee_tiles(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_erpx_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    if apps.is_installed("payroll"):
        Reimbursement = get_erpx_model_class(

...
from erpx.settings import TIME_ZONE
from erpx.erpx_settings import BIO_DEVICE_THREADS
...
@install_required
@hx_request_required
def biometric_device_log_fetch(request, device_id):
    ...
    count = logs.get("found")

    if logs.get("status_code") and logs.get("status_code") == 200:
        dahua_biometric_device_attendance(logs, device_id)
    ...
    script = (
        ...
        if logs.get("status_code") and logs.get("status_code") != 200
        else render_connection_response(
            _("Success"),
            _(
                "{} attendance logs have been successfully fetched from the device."
            ).format(count),
            "success",
        )
    )
...
@install_required
@permission_required("biometric.delete_biometricemployees")
def delete_erpx_cosec_user(request, user_id, device_id):
    """
...
def dahua_biometric_device_attendance(logs, device_id):
    device = BiometricDevices.find(device_id)
    ...
    if device:
        for log in logs.get("records"):
            if (
                log.get("user_id")
                and BiometricEmployees.objects.filter(
                    user_id=log.get("user_id"), device_id=device
                ).first()
            ):
                employee = BiometricEmployees.objects.filter(
        ...
        if logs.get("records"):
            last_log = logs.get("records")[-1]
            attendance_datetime = last_log.get("create_time")
            device.last_fetch_date = attendance_datetime.date()
            device.last_fetch_time = attendance_datetime.time()
            device.save()

...
try:
    ...
    erpx_mail_templates = erpxMailTemplate.objects.all()
...
try:
    ...
    for penalty in penalties:
        ...
        if not PenaltyAccounts.objects.filter(**filter_conditions).exists():
            erpx = PenaltyAccounts(
                id=penalty.id,
                employee_id=penalty.employee_id,
                penalty_amount=penalty.penalty_amount,
            )
            if apps.is_installed("attendance"):
                erpx.late_early_id = penalty.late_early_id
            if apps.is_installed("leave"):
                erpx.leave_request_id = penalty.leave_request_id
                erpx.leave_type_id = penalty.leave_type_id
                erpx.minus_leaves = penalty.minus_leaves
                erpx.deduct_from_carry_forward = penalty.deduct_from_carry_forward
            erpx.save()

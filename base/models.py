...
class JobPosition(erpxModel):
    ...
    objects = erpxCompanyManager("department_id__company_id")
...
class JobRole(erpxModel):
    ...
    objects = erpxCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(erpxModel):
    """
    ...
    objects = erpxCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(erpxModel):
    """
...
class EmployeeShift(erpxModel):
    ...
    objects = erpxCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(erpxModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the erpx will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = erpxCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShiftAssign(erpxModel):
    """
...
class WorkTypeRequest(erpxModel):
    """
...
class WorkTypeRequestComment(erpxModel):
    """
...
class ShiftRequestComment(erpxModel):
    """
...
class Tags(erpxModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = erpxCompanyManager(related_company_field="company_id")

    def __str__(self):
        return self.title


class erpxMailTemplate(erpxModel):
...
class DynamicEmailConfiguration(erpxModel):
    """
...
class MultipleApprovalCondition(erpxModel):
...
class AnnouncementComment(erpxModel):
    """
...
class DashboardEmployeeCharts(erpxModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(erpxModel):
...
class PenaltyAccounts(erpxModel):
    """
...
def create_deduction_cutleave_from_penalty(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if apps.is_installed("payroll") and penalty_amount:
            Deduction = get_erpx_model_class(app_label="payroll", model="deduction")
            penalty = Deduction()
